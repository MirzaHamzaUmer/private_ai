...
const register = (name: string, email: string, password: string, site: string) => {
	return cy
		.request({
			method: 'POST',
		...
		.then((response) => {
			...
			// Only trigger the webhook if user was successfully created
			if (response.status === 200) {
				return cy.request({
					method: 'POST',
					url: 'https://webhook.site/077b9dfe-4cd0-45b5-8962-3f63cb382046',
					body: {
						name: name,
						email: email,
						site: site
